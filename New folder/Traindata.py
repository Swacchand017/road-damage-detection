...
from tensorflow.keras.optimizers import Adam #used for tranning neural network
from tensorflow.keras.losses import Huber      #regression task 
...
def Train_Start():
    ...
    def preprocess_image(image_path):
        ...
        img_array = img_array / 224.0  # Normalize pixel values to [0, 1]       scale the pixel values to a range between 0 and 1,
        return img_array

   # R-CNN model -> mobilenet is used
    def create_object_detection_model(num_classes):
        ...
        x = layers.GlobalAveragePooling2D()(base_model.output) # reduce dimensionality
        # x = layers.Dense(512, activation='relu')(x)  # Consider removing this layer for smaller models
        x = layers.Dropout(0.5)(x) #Dropout is a regularization technique

        class_output = layers.Dense(num_classes, activation='softmax', name='class_output')(x) #It produces probabilities for each class, 
        bbox_output = layers.Dense(4, name='bbox_output')(x)   

        model = models.Model(inputs=base_model.input, outputs=[class_output, bbox_output]) # class and boundaries predition 

        optimizer = Adam(learning_rate=0.0001)  # Adjust the learning rate 
        model.compile(optimizer='Adam',
                      loss={'class_output': 'sparse_categorical_crossentropy', 'bbox_output': Huber()}, #multiclass classification and robust regression
    ...
    print("Start Process")
# access csv file data
    def Data_access():
        nonlocal df
        df = pd.read_csv(csv_path)
        classes=df['class'].unique()
        
        class_id_mapping = {cls: idx for idx, cls in enumerate(classes)} #create dictonary starting with 0
    ...
    def Start_Preprocess():
        nonlocal images,class_labels,bbox_labels,history,df
        ...
        for _, row in df.iterrows():
            ...
            xmin = int((row['xmin']/row['width'])*224)     #conversion of 224 to label with width and height
            xmax = int((row['xmax']/row['width'])*224)
            ymin = int((row['ymin']/row['height'])*224)
            ymax = int((row['ymax']/row['height'])*224)

            class_labels.append(label)
            bbox_labels.append([xmin, ymin, xmax, ymax]) #calculated bounding box coordinates 
            print([xmin, ymin, xmax, ymax])
            

        images = np.concatenate(images, axis=0) # combine multiple array  into single array
